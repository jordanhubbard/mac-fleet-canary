...
def tokenize(expression: str) -> list[Token]:
    ...
    contain at least one ASCII digit. Minus is always emitted separately,
    allowing a parser to distinguish unary negation from subtraction.
    """
    ...
    while position < len(expression):
        ...
        if character in "0123456789" or (
            character == "."
            and position + 1 < len(expression)
            and expression[position + 1] in "0123456789"
        ):
            start = position
